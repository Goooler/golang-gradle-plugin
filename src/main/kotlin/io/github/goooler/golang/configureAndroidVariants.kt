...
internal fun Project.configureAndroidVariants(goExtension: GoExtension) {
  ...
  androidComponents.onVariants { variant ->
    ...
    configureCMakeTasks(compileTasks)
  }
}

/**
 * There is no official way to add task dependencies to CMake build tasks, so we have to rely on the
 * task name pattern. This is necessary for headers generated by Go to be available when CMake
 * builds the native library.
 */
private fun Project.configureCMakeTasks(compileTasks: Map<String, TaskProvider<GoCompile>>) {
  tasks
    ...
    .configureEach { cmake ->
      compileTasks.forEach { (abi, goCompile) ->
        ...
        val cmakeName = cmake.name
        val goTaskName = goCompile.name

        // `buildCMakeDebug[arm64-v8a]` for debug.
        val isDebugMatch = cmakeName.contains("Debug") && goTaskName.contains("Debug")
        // `buildCMakeRelWithDebInfo[arm64-v8a]` for release.
        val isReleaseMatch =
          (cmakeName.contains("Release") ||
            cmakeName.contains("RelWithDebInfo") ||
            cmakeName.contains("MinSizeRel")) && goTaskName.contains("Release")
